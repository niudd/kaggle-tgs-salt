...
def _do_center_pad_to_factor256(image):
    x202 = cv2.resize(image, dsize=(202, 202))
    x256 = cv2.copyMakeBorder(x202, 27, 27, 27, 27, cv2.BORDER_REFLECT)
    return x256

def do_center_pad_to_factor256(image, mask):
    image = _do_center_pad_to_factor256(image)
    mask = _do_center_pad_to_factor256(mask)
    return image, mask
